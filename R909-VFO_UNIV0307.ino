// 2025.03.09 temporary releases 100kHz-200MHz V2.1
...
//  Frequency coverage 118-136Mz L361,L189,L1397,L1422  FOR 2nd radio
//   BAND
//  Step coverage 10,100,1000,9000?,10000,10000Hz L372,L383,L1373
//  
//
// --=----=----=----=----=----=----=----=----=----=----=----=----
...
//  getSettings();
  ...
  if (freq1 < 100000 || freq1 > 200000000) { //100kHz-200MHz
    freq1 = 10000000;
    putFirstSettings();
    EEPROM.put( FREQ_AD, freq1);      // FREQ data  10MHz for EPROM
...
void loop() {
  ...
  blink(300, 400);      // Blinking for monitor LED and FUNC display on OLED
...
// display  
  if(Timer_DISP_REF<=0){      // Periodic display timer expired
    ...
    layout();
    display.clearDisplay();    
    Timer_DISP_REF = 300;    // Display every 300mS
  }
...
void s_meter_Disp()   {
  ...
  s_value = map(rssi, 0, 1024, 0, 9);     // s max changed
  ...
  if (mode == FREQ) {                    // FREQ:mode 0 : Up/Down frequency
    if (dir == 0x10){                    // Clockwise
      ...
      if (BandSelect == true) {          // AM mode limit
        if (freq1 > (long) 200000000) {  //  upper -> 200MHz
          freq1 = (long)   200000000;
        }
      }
